class Hold {
  ...
  move(speed) {
    ...
    if(this.y - this.height < 0){
      ...
      this.note.style.height = this.height - Math.abs(this.y - this.height) + "px"
    }
    ...
    if (this.y > this.line.clientHeight && this.status != "RELEASED") {
      if(this.y - this.height < 0){
        ...
        this.note.style.height = this.line.clientHeight - (this.y - this.height) + "px";
      }
    }
  }
  ...
  getHoldTopStatus(timing) {
    ...
    if (Math.abs(timing - this.targetTiming) < this.PERFECT) return "HOLD_TOP_PERFECT";
    if (timing - this.targetTiming > this.PERFECT) return "HOLD_TOP_MISS";
    return "UNREACHED";
  }

  getHoldStatus(timing){
    if(timing > this.targetTiming + this.lastTime - this.RELEASE) return "FINISH";
    return this.status;
  }
